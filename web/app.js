...
const app = {
  ...
  loadExamsDataset: async function() {
    try {
      ...
      try {
        const response = await fetch('../data/exams_50_dataset.json');
        const json = await response.json();
        this.data.exams = json.exams || [];
      } catch (err) {
  ...
  updateUserStatsDisplay: function() {
    ...
    if (user) {
      ...
      if (heroBadge) {
        heroBadge.innerHTML = `<i data-lucide="user-check" class="w-3.5 h-3.5 text-indigo-400"></i> Đang đăng nhập: <strong>${user.name}</strong>`;
      }

      if (isActive) {
        if (subBadge) {
          subBadge.classList.remove('hidden');
          subBadge.className = 'flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-indigo-950/80 border border-indigo-700 text-cyan-300 text-xs font-bold shadow-sm';
        }
        ...
        if (statusBadge) {
          ...
          statusBadge.innerHTML = '<i data-lucide="unlock" class="w-4 h-4"></i> Đề 01 Sẵn Sàng Làm Bài';
        }
      } else {
        if (subBadge) {
          subBadge.classList.remove('hidden');
          subBadge.className = 'flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-rose-950 border border-rose-700 text-rose-300 text-xs font-bold animate-pulse';
        }
        if (subText) subText.innerText = '⚠️ Tài khoản đã Hết Hạn';
        if (statusBadge) {
          ...
          statusBadge.innerHTML = '<i data-lucide="alert-circle" class="w-4 h-4"></i> Tài Khoản Hết Hạn';
        }
      }
        ...
        authSec.innerHTML = `
          <div class="flex items-center gap-2">
            <div class="flex items-center gap-1.5 p-1 pl-2 pr-2.5 rounded-xl bg-slate-800 border border-slate-700">
              <div class="w-6 h-6 rounded-lg bg-indigo-500 flex items-center justify-center font-bold text-xs text-white">
                ${user.name.charAt(0).toUpperCase()}
              </div>
              <span class="text-xs font-bold text-slate-100 hidden md:inline-block">${user.name}</span>
            </div>
            <button onclick="app.logout()" class="p-2 rounded-xl bg-slate-800 hover:bg-rose-950 text-slate-400 hover:text-rose-300 border border-slate-700 hover:border-rose-800 transition-colors" title="Đăng Xuất">
      ...
      if (heroBadge) {
        heroBadge.innerHTML = `<i data-lucide="user-x" class="w-3.5 h-3.5 text-slate-400"></i> Vui lòng đăng nhập để bắt đầu`;
      }
      ...
      if (statusBadge) {
        ...
        statusBadge.innerHTML = '<i data-lucide="lock" class="w-4 h-4"></i> Đăng Nhập Để Làm Bài';
      }
      if (authSec) {
        authSec.innerHTML = `
          <button onclick="app.openLoginModal()" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-lg shadow-indigo-600/30 flex items-center gap-1.5 transition-all">
  ...
  handleLogin: function(event) {
    ...
    if (user) {
      ...
      this.updateUserStatsDisplay();
      this.playSound('pass');
      alert(`✅ Đăng nhập thành công! Chào mừng học viên ${user.name} vào luyện thi.`);
    } else {
      this.playSound('fail');
      alert('❌ Tên đăng nhập hoặc mật khẩu không chính xác!\n\nChỉ có Admin mới có quyền cấp tài khoản. Vui lòng liên hệ Admin qua Cửa Hàng (binhluu.ai.studio) để mua bản quyền và nhận tài khoản.');
    }
  },
  ...
  renderRoadmap: function() {
    ...
    filtered.forEach(ex => {
      ...
      card.className = `glass-card rounded-3xl p-5 sm:p-6 flex flex-col justify-between ${
      ...
      card.innerHTML = `
        <div class="space-y-3.5">
          <div class="flex items-center justify-between">
            <span class="px-3 py-1 rounded-full text-xs font-bold border ${levelBadge}">
              ${ex.level}
            </span>
            <div class="flex items-center gap-1.5 text-xs font-bold">
              ${
                isPassed 
                  ? `<span class="text-emerald-400 flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-950 border border-emerald-800"><i data-lucide="check-circle-2" class="w-3.5 h-3.5"></i> ${passInfo.score}%</span>`
                  : isUnlocked 
                    ? `<span class="text-cyan-400 flex items-center gap-1 px-2.5 py-1 rounded-full bg-cyan-950 border border-cyan-800"><i data-lucide="unlock" class="w-3.5 h-3.5"></i> Mở</span>` 
                    : `<span class="text-slate-500 flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-900 border border-slate-800"><i data-lucide="lock" class="w-3.5 h-3.5"></i> Khóa</span>`
              }
            </div>
          </div>
        ...
        <div class="mt-5 pt-4 border-t border-slate-800 flex items-center justify-between">
          ...
          ${
            isLoggedIn
              ? isUnlocked
                ? `<button onclick="app.startExam('${ex.exam_id}')" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-md flex items-center gap-1.5 transition-all">
                    ${isPassed ? 'Làm Lại' : 'Bắt Đầu'} <i data-lucide="play" class="w-3.5 h-3.5"></i>
                  </button>`
                : `<button disabled class="px-4 py-2 rounded-xl bg-slate-900 text-slate-500 font-semibold text-xs cursor-not-allowed flex items-center gap-1.5 border border-slate-800">
                    <i data-lucide="lock" class="w-3.5 h-3.5"></i> Chưa Mở Khóa
                  </button>`
              : `<button onclick="app.openLoginModal()" class="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-cyan-300 font-bold text-xs flex items-center gap-1.5 border border-slate-700">
  ...
  renderContinuousExamSheet: function() {
    ...
    sec1.className = 'space-y-6 pt-2';
    sec1.innerHTML = `
      <div class="flex items-center gap-3 p-4 rounded-2xl bg-indigo-950/70 border border-indigo-700/80">
      ...
      partCard.innerHTML = `
        ...
        <div class="space-y-5">
          ${part.questions.map(q => {
            ...
            return `
              <div class="space-y-3 bg-slate-950 p-4 sm:p-5 rounded-2xl border border-slate-800">
                <div class="font-semibold text-sm sm:text-base text-white flex items-start gap-2.5 leading-snug">
                  <span class="px-2.5 py-0.5 rounded-lg bg-indigo-900 text-cyan-300 text-xs font-bold shrink-0 mt-0.5">Câu ${currentNum}</span>
                  ...
                  ${q.options.map(opt => {
                    ...
                    return `
                      <label class="option-label">
                        ...
                        <span class="text-sm font-normal text-slate-100">${opt}</span>
                      </label>
                    `;
                  }).join('')}
                </div>
    ...
    sec2.id = 'sec-reading';
    sec2.className = 'space-y-6 pt-4';
    sec2.innerHTML = `
      <div class="flex items-center gap-3 p-4 rounded-2xl bg-cyan-950/70 border border-cyan-700/80">
        <div class="w-10 h-10 rounded-xl bg-cyan-600 flex items-center justify-center text-slate-950 shadow-md">
      ...
      partCard.innerHTML = `
        ...
        <div class="space-y-5">
          ${part.questions.map(q => {
            ...
            return `
              <div class="space-y-3 bg-slate-950 p-4 sm:p-5 rounded-2xl border border-slate-800">
                ${q.context ? `<div class="p-3.5 rounded-2xl bg-indigo-950/60 border border-indigo-700/80 text-xs sm:text-sm font-mono text-cyan-200 mb-3">${q.context}</div>` : ''}
                ${q.passage ? `<div class="p-4 rounded-2xl bg-slate-900 border border-slate-700 text-xs sm:text-sm text-slate-100 leading-relaxed font-normal mb-3">${q.passage}</div>` : ''}
                <div class="font-semibold text-sm sm:text-base text-white flex items-start gap-2.5 leading-snug">
                  <span class="px-2.5 py-0.5 rounded-lg bg-cyan-950 text-cyan-300 text-xs font-bold shrink-0 mt-0.5">Câu ${currentNum}</span>
                  <span>${q.question}</span>
                </div>
                <div class="grid grid-cols-1 gap-2.5 pt-1">
                  ${q.options.map(opt => {
    ...
    wCard1.className = 'glass-panel rounded-3xl p-5 sm:p-7 space-y-5';
    wCard1.innerHTML = `
      <div class="pb-3 border-b border-slate-800">
        <h4 class="font-bold text-base sm:text-lg text-white">${wPart1.title}</h4>
      ...
      <div class="space-y-4">
        ${wPart1.questions.map(q => {
          const currentNum = questionGlobalIndex++;
          return `
            <div class="space-y-2 bg-slate-950 p-4 sm:p-5 rounded-2xl border border-slate-800">
              <div class="text-xs sm:text-sm text-slate-300">
                ...
                Câu gốc: <strong class="text-white">"${q.original}"</strong> (Từ cho sẵn: <strong class="text-amber-400 font-mono text-sm">${q.target_word}</strong>)
              </div>
              <div class="text-sm sm:text-base font-semibold text-slate-100 mt-1">${q.prompt}</div>
              <input type="text" placeholder="Nhập từ còn thiếu vào đây..." value="${this.data.userAnswers[q.id] || ''}" oninput="app.recordAnswer('${q.id}', this.value)" class="w-full mt-2 px-4 py-3 rounded-xl bg-slate-900 border-2 border-slate-700 text-sm sm:text-base text-white focus:outline-none focus:border-amber-400 font-medium transition-colors">
            </div>
          `;
        }).join('')}
      </div>
    ...
    wCard2.innerHTML = `
      ...
      <div class="p-4 sm:p-5 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
  ...
  showResultModal: function(score, isPassed, lis, rea, wri, spk) {
    ...
    if (isPassed) {
      card.className = 'rounded-3xl p-6 sm:p-8 border border-emerald-500/50 bg-gradient-to-b from-emerald-950/50 via-slate-900 to-slate-950 shadow-2xl relative overflow-hidden text-center space-y-5';
      icon.className = 'w-16 h-16 mx-auto rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center text-3xl shadow-xl border border-emerald-500/30';
      icon.innerHTML = '🎉';
      title.innerText = 'CHÚC MỪNG BẠN ĐÃ VƯỢT ẢI THÀNH CÔNG!';
      title.className = 'font-extrabold text-2xl sm:text-3xl text-emerald-300';
      sub.innerHTML = `Bạn đã xuất sắc đạt <strong>${score}%</strong> (vượt ngưỡng yêu cầu 50%). Bộ đề tiếp theo đã được mở khóa!`;
      nextBtn.classList.remove('hidden');
    } else {
      card.className = 'rounded-3xl p-6 sm:p-8 border border-rose-500/50 bg-gradient-to-b from-rose-950/50 via-slate-900 to-slate-950 shadow-2xl relative overflow-hidden text-center space-y-5';
      icon.className = 'w-16 h-16 mx-auto rounded-full bg-rose-500/20 text-rose-400 flex items-center justify-center text-3xl shadow-xl border border-rose-500/30';
      icon.innerHTML = '⚠️';
      title.innerText = 'CHƯA ĐẠT NGƯỠNG VƯỢT ẢI (50%)';
      title.className = 'font-extrabold text-2xl sm:text-3xl text-rose-300';
  ...
  renderReviewDetails: function() {
    ...
    exam.skills.listening.parts.forEach(p => {
      p.questions.forEach(q => {
        ...
        el.className = `p-5 sm:p-6 rounded-3xl border-2 ${isCorrect ? 'bg-emerald-950/40 border-emerald-700/80' : 'bg-rose-950/40 border-rose-700/80'} space-y-3.5 shadow-lg`;

        el.innerHTML = `
          <div class="flex items-center justify-between">
            <span class="text-xs sm:text-sm font-bold px-3 py-1 rounded-full ${isCorrect ? 'bg-emerald-900 text-emerald-200 border border-emerald-600' : 'bg-rose-900 text-rose-200 border border-rose-600'}">
              ${isCorrect ? '✓ TRẢ LỜI ĐÚNG' : '✗ TRẢ LỜI SAI'}
            </span>
            <span class="text-xs sm:text-sm text-slate-300 font-bold">${p.title}</span>
          </div>

          <div class="font-semibold text-sm sm:text-base text-white leading-snug">${q.question}</div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs sm:text-sm">
            <div class="p-3 rounded-2xl bg-slate-900 border border-slate-700">
              <span class="text-slate-300 font-medium">Bạn đã chọn:</span> <strong class="${isCorrect ? 'text-emerald-400' : 'text-rose-400'} font-bold text-sm sm:text-base ml-1">${userAns || 'Chưa chọn'}</strong>
            </div>
            <div class="p-3 rounded-2xl bg-slate-900 border border-slate-700">
              <span class="text-slate-300 font-medium">Đáp án đúng:</span> <strong class="text-emerald-400 font-bold text-sm sm:text-base ml-1">${q.correct_answer}</strong>
            </div>
          </div>

          <div class="p-4 rounded-2xl bg-slate-950 border border-slate-800 text-xs sm:text-sm text-slate-100 space-y-1.5 leading-relaxed font-normal">
            <div class="text-cyan-300 font-bold flex items-center gap-1.5">💡 Giải thích chi tiết:</div>
            <div>${q.explanation}</div>
            ${q.tapescript ? `<div class="mt-2 text-slate-300 italic p-2.5 rounded-xl bg-slate-900/80 border border-slate-800">🎧 Tapescript: "${q.tapescript}"</div>` : ''}
          </div>
        `;
  ...
  renderHistory: function() {
    ...
    if (!this.data.currentUser) {
      list.innerHTML = `
        <div class="text-center py-12 bg-slate-900/40 rounded-3xl border border-slate-800 text-slate-300 text-sm font-medium">
    ...
    attempts.forEach(att => {
      const el = document.createElement('div');
      el.className = 'p-5 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-md';
      el.innerHTML = `
        <div>
          <div class="font-bold text-sm text-white">${att.title}</div>
          <div class="text-xs text-slate-400 mt-0.5">Ngày thi: ${att.date} • Mã đề: ${att.exam_id}</div>
        </div>
        <div class="flex items-center gap-3">
          <span class="text-sm font-bold ${att.passed ? 'text-emerald-400' : 'text-rose-400'}">${att.score} Điểm (${att.passed ? 'ĐẠT' : 'CHƯA ĐẠT'})</span>
        </div>
      `;
      list.appendChild(el);
    });
